...
class VoxelGrid(nn.Module):
    ...
    def coords_to_bounding_voxel_grid(self, coords, coord_features=None,
        ...
        if coord_features is not None:
            voxel_values = torch.cat([voxel_values, coord_features], -1) # [B,Num_points,3 + feature_dim]
        ...
        voxel_values_pruned_flat = torch.cat(
            [voxel_values, self._ones_max_coords[:, :num_coords]], -1) # [B,Num_point,3 + feature_dim + 1]
        ...
        vox = scattered[:, 1:-1, 1:-1, 1:-1] # [3,34,34,34,132]
